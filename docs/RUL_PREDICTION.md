# Experiment C â€” Does the Forecastable Health State Predict RUL?
...
**Artifacts:** `results/tables/FD001/rul_predictions.csv`, `results/tables/FD001/rul_metrics.json`

---

## 1. Falsifiable hypothesis

> **H.** Using only the identified, forecastable health state â€” current level
> $h_0,h_1$ and forecast velocity $\dot h_0,\dot h_1$ â€” RUL on the official
...
### (i) Naive forecast-to-threshold â€” *and why it fails*
...
$\approx 0.178$ (start) to $\approx 0.215$ (failure) â€” a dynamic range of
...
This is reported transparently â€” it is the empirical signature of the
*latent-scaling* issue, not a defect of the health state itself.

### (ii) Robust health$\to$RUL map â€” the estimator a PHM engineer would use
...
$\dot h_0$ â€” the very quantity Experiment B proved forecastable â€” is a feature.
...
|---|---|---|---|---|
| **health â†’ RUL (robust)** | **13.66** | **10.35** | **+0.892** | **327** |
| threshold-crossing (naive) | 49.16 | 37.44 | âˆ’0.399 | 662 829 |
| mean-RUL baseline | 43.07 | 35.90 | âˆ’0.074 | 33 629 |
...
![Predicted vs true RUL and error histogram](../results/figures/FD001/C1_rul_scatter.png)

### 3.2 The learned health â†’ RUL relationship is monotone (Proposition 5)

Binned $\mathbb{E}[\text{RUL}\mid h_0]$ falls cleanly from $\sim\!105$ to
$\sim\!7$ cycles as $h_0$ rises â€” the monotone map guaranteed by Proposition 5.
...
![Health level vs RUL relationship](../results/figures/FD001/C3_health_vs_rul.png)
...
![Example engines: causal health and RUL readout](../results/figures/FD001/C2_examples.png)
...
| Claim | Outcome |
|---|---|
| Beats mean baseline on RMSE | **Confirmed** â€” 13.7 vs 43.1 (âˆ’68 %). |
| Beats baseline on NASA score | **Confirmed** â€” 327 vs 33 629. |
| No leakage (causal, official test) | **Confirmed** by construction. |
| Naive threshold-crossing works | **Refuted** â€” documented latent-scaling failure. |
...
RMSE 13.7 (Exp C). The one honest limitation â€” naive threshold-crossing on the
raw latent â€” is a scaling artifact of an unsupervised bottleneck, fixed simply
...
- FD001 is the single-condition / single-fault subset. The same protocol should
  be re-run on FD002â€“FD004 (multiple operating regimes) to test robustness.
